...
class Visualize:
    ...
    def show_gc_positions_and_density(self, rho, phi, gc_density_func_params):
        ...
        index = np.all([phi > np.min(self.theta), phi < np.max(self.theta),
                        rho > np.min(self.eccentricity_in_mm), rho < np.max(self.eccentricity_in_mm)],
                       axis=0)  # Index only cells within original requested theta
        ...
        area_for_each_bin = self.sector2area(bin_edges[1:], np.ptp(self.theta)) \
                            - self.sector2area(bin_edges[:-1],
                                               np.ptp(self.theta))  # in mm2. Vector length len(edge_ecc) - 1.
        # Cells/area
    ...
    def show_dendritic_diameter_vs_eccentricity(self, gc_type, dataset_x, dataset_y, polynomials, dataset_name=''):
        ...
        if dataset_name:
            if len(polynomials) == 2:  # check if only two parameters, ie intercept and slope
                ...
                cube = polynomials[0]
                ax.plot(dataset_x, intercept + slope * dataset_x + square * dataset_x ** 2 + cube * dataset_x ** 3,
                        "k--")
                ax.annotate(
                    "{0}: \ny={1:.1f} + {2:.1f}x + {3:.1f}x^2 + {4:.1f}x^3".format(dataset_name, intercept, slope,
                                                                                   square, cube),
                    xycoords='axes fraction', xy=(.5, .15), ha="left", color='k')
