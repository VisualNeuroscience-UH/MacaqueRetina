...
'''
This module was originally copied from https://github.com/jleinonen/keras-fid.
It was published under MIT license.

Modified by Simo Vanni 10/2022	
'''
...
class FrechetInceptionDistance(object):
	...
	Class for evaluating images using the Frechet Inception Distance (Heusel et al. 2017, 
	...
	Arguments to call:
		real_images: An 4D NumPy array of images from the training dataset,
			...
			Inception network).
		generated_images: A numpy array of images generated by the generator,
		...
		shuffle: If True (default), samples are randomly selected from the
			input arrays. No effect if real_images or generated_images is
			a Python generator.
	...
	def __init__(self, image_range=None, 
		generator_postprocessing=None):

		assert image_range is not None, \
			'''image_range missing: A tuple giving the range of values in the images. This is used to rescale to the (-1,1) range
			expected by the Inception V3 network.'''
	...
	def _setup_inception_network(self):
		# Data has been upsampled to min shape accepted by the Inception network
		self._inception_v3 = InceptionV3(include_top=False, pooling='avg', input_shape=(75,75,3))
	...
	def _stats(self, inputs, input_type="real", postprocessing=None,
		...
		for i in range(num_batches):
			...
			# We pass upsampled generated images to the Inception network
	...
	def __call__(self,
		...
		(gen_mean, gen_cov) = self._stats(generated_images,
