...
class Cones(ReceptiveFieldsBase):
    ...
    def create_noise(self, vs, n_trials):
        ...
        vs.gc_synaptic_noise = gc_synaptic_noise.T
...
class Bipolars(ReceptiveFieldsBase):
    ...
    def create_signal(self, vs):
        ...
        Notes
        -----
        This function uses predefined weights and nonlinearity parameters stored in an npz file accessible via the instance. The steps include:
        - Get Rectifying Index as parabolic function of surround stimulus.
        - Calculating bipolar cell inputs by matrix multiplication of cone outputs and weights.
        - Applying synaptic input scaling for negative responses (rectification/nonlinearity).
        """
...
class SimulateRetina(RetinaMath):
    ...
    def _generator_to_firing_rate_noise(self, vs, gcs, n_trials):
        """
        ...
        if n_trials > 1:
            raise NotImplementedError("Multiple trials not yet implemented")

        gc_noise_mean = params_all.Mean.values
        firing_rates_cone_noise = vs.gc_synaptic_noise * gc_noise_mean[:, np.newaxis]

        gc_gain = params_all.A.values
        firing_rates_light = vs.generator_potentials * gc_gain[:, np.newaxis]
        # # breakpoint()
        # fig, ax = plt.subplots(2, 1)
        # ax[0].hist(vs.generator_potentials.flatten(), 100)
        # ax[1].hist(vs.gc_synaptic_noise.flatten(), 100)
        # # [ax[i].legend() for i in range(ax.size)]
        # plt.show()
    ...
    def _add_noise_to_generator_potential(self, vs, gcs, n_trials):
        """
        Generates cone noise, scales it with mean firing rates. Multiplies the generator
        potentials with gain and finally adds the firing rates generated by the cone noise
        to the light-induced firing rates.
